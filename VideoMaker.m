function VideoMaker(coords, conecs, u, failStat, Tsteps, OutputsName, FrameRate, A_t)
% This function makes a video of the structure's deformation throughout
% the analysis.
...
%         min(coords_istep(:, 2))-edge max(coords_istep(:, 2))+edge])
    Drawdef(coords, conecs, reshape(u(i, :), 2, '')', failStat(i, :), A_t);
    axis off
